    ...
    static findChartCanvas() {
        ...
        for (const selector of selectors) {
            ...
            if (canvas && canvas.width > 300 && canvas.height > 200) {
                logger.info(`Found chart canvas: ${selector}`);
                return canvas;
            }
        }
        ...
        const largestCanvas = allCanvases.reduce((largest, canvas) => {
        ...
        if (largestCanvas) {
            logger.info('Found largest canvas as fallback');
            return largestCanvas;
        }

        logger.warn('No suitable canvas found');
        return null;
    }
